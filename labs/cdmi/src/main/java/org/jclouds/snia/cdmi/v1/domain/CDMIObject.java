...
public class CDMIObject {
   ...
   public abstract static class Builder<B extends Builder<B>> {
      ...
      private List<AccessControlElement> acl;
      ...
      public B acl(List<AccessControlElement> acl) {
         this.acl = ImmutableList.copyOf(checkNotNull(acl, "acl"));
         return self();
      }
               ...
               for (int i = 0; i < cdmi_acl_array.length; i++) {
                  if (!(cdmi_acl_array[i].startsWith("[") || cdmi_acl_array[i].startsWith("]") || cdmi_acl_array[i]
                     ...
                     for (String s : cdmi_acl_member) {
                        ...
                        cdmi_acl_key = cdmi_acl_key.replace('"', ' ').trim();
                     }
                     acl.add(new AccessControlElement(aclMap));
                  }
               }
            } else {
    ...
    * @return list of Access Control Elements
    */
   public List<AccessControlElement> getAccessControlList() {
      if (acl == null) {
         parseMetadata();
      }
      return acl;
